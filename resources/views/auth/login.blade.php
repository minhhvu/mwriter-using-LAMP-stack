    ...
    <div class="">
        <div class="row justify-content-center">
            <div class="col-md-9 mt-5 mb-4">
                <div class="card">
{{--                    @include('components.brandview')--}}

                    <div class="card-body">
                        @if (session()->has('login_error'))
                            <div class="alert alert-success">{{session()->get('login_error')}}</div>
                        @endif
                        <form method="POST" action="{{ route('login') }}" aria-label="{{ __('Login') }}">
                            @csrf

                            <div class="form-group row">
                                <label for="email" class="col-sm-4 col-form-label text-md-right">{{ __('Email or Username') }}</label>
